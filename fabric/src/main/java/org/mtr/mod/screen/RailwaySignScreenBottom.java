...
import org.mtr.mod.packet.PacketUpdateRailwaySignBottomConfig;
import org.mtr.mod.render.RenderRailwaySignBottom;
import org.mtr.mod.resource.SignResource;

import javax.annotation.Nullable;

public class RailwaySignScreenBottom extends ScreenExtension implements IGui {
	...
	public RailwaySignScreenBottom(BlockPos signPos) {
		super();
		editingIndex = -1;
		...
		if (world != null) {
			...
			if (entity != null && entity.data instanceof BlockRailwaySignBottom.BlockEntity) {
				signIds = ((BlockRailwaySignBottom.BlockEntity) entity.data).getSignIds();
				selectedIds = ((BlockRailwaySignBottom.BlockEntity) entity.data).getSelectedIds();
				isRailwaySign = true;
			} else {
			...
			if (block.data instanceof BlockRailwaySignBottom) {
				length = ((BlockRailwaySignBottom) block.data).length;
			} else {
				length = 0;
			}
		} else {
	...
	public void render(GraphicsHolder graphicsHolder, int mouseX, int mouseY, float delta) {
		...
		if (editingIndex >= 0) {
			...
			loopSigns((index, x, y, isBig) -> {
				...
				final SignResource sign = RenderRailwaySignBottom.getSign(signId);
				if (sign != null) {
					final boolean moveRight = sign.hasCustomText && sign.getFlipCustomText();
					RenderRailwaySignBottom.drawSign(graphicsHolder, null, signPos, signId, (isBig ? xOffsetBig : xOffsetSmall) + x + (moveRight ? SIGN_BUTTON_SIZE * 2 : 0), BUTTON_Y_START + y, SIGN_BUTTON_SIZE, 2, 2, selectedIds, Direction.UP, 0, (textureId, x1, y1, size, flipTexture) -> {
						final GuiDrawing guiDrawing = new GuiDrawing(graphicsHolder);
	...
	public void onClose2() {
		RegistryClient.sendPacketToServer(new PacketUpdateRailwaySignBottomConfig(signPos, selectedIds, signIds));
		super.onClose2();
	}
	...
	private int loopSigns(LoopSignsCallback loopSignsCallback, boolean ignorePage) {
		...
		for (int i = 0; i < allSignIds.size(); i++) {
			final SignResource sign = RenderRailwaySignBottom.getSign(allSignIds.get(i));
