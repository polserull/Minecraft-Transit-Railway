...
public class BlockRailwaySignBottom extends BlockExtension implements IBlock, DirectionHelper, BlockWithEntity {
	...
	public BlockRailwaySignBottom(int length, boolean isOdd) {
	...
	public BlockState getStateForNeighborUpdate2(BlockState state, Direction direction, BlockState neighborState, WorldAccess world, BlockPos pos, BlockPos neighborPos) {
		final Direction facing = IBlock.getStatePropertySafe(state, FACING);
		final boolean isNext = direction == facing.rotateYClockwise() || state.isOf(org.mtr.mod.Blocks.RAILWAY_SIGN_BOTTOM_MIDDLE.get()) && direction == facing.rotateYCounterclockwise();
		if (isNext && !(neighborState.getBlock().data instanceof BlockRailwaySignBottom)) {
	...
	public void onPlaced2(World world, BlockPos pos, BlockState state, @Nullable LivingEntity placer, ItemStack itemStack) {
		if (!world.isClient()) {
			...
			for (int i = 1; i <= getMiddleLength(); i++) {
				world.setBlockState(pos.offset(facing.rotateYClockwise(), i), org.mtr.mod.Blocks.RAILWAY_SIGN_BOTTOM_MIDDLE.get().getDefaultState().with(new Property<>(FACING.data), facing.data), 3);
			}
	...
	public VoxelShape getOutlineShape2(BlockState state, BlockView world, BlockPos pos, ShapeContext context) {
		...
		if (state.isOf(org.mtr.mod.Blocks.RAILWAY_SIGN_BOTTOM_MIDDLE.get())) {
			return IBlock.getVoxelShapeByDirection(0, 4, 7, 16, 16, 9, facing);
		} else {
			final int xStart = getXStart();
			final VoxelShape main = IBlock.getVoxelShapeByDirection(xStart - 0.75, 4, 7, 16, 16, 9, facing);
			final VoxelShape pole = IBlock.getVoxelShapeByDirection(xStart - 2, 4, 7, xStart - 0.75, 16, 9, facing);
	...
	public void addTooltips(ItemStack stack, @Nullable BlockView world, List<MutableText> tooltip, TooltipContext options) {
		...
		tooltip.add(TextHelper.translatable("Type: Bottom").formatted(TextFormatting.GRAY));
	}

	@Nonnull
	@Override
	public BlockEntityExtension createBlockEntity(BlockPos blockPos, BlockState blockState) {
		if (this == org.mtr.mod.Blocks.RAILWAY_SIGN_BOTTOM_MIDDLE.get().data) {
			return null;
		} else {
	...
	private BlockPos findEndWithDirection(World world, BlockPos startPos, Direction direction, boolean allowOpposite) {
		...
		while (true) {
			...
			if (checkState.getBlock().data instanceof BlockRailwaySignBottom) {
				final Direction facing = IBlock.getStatePropertySafe(checkState, FACING);
				if (!checkState.isOf(org.mtr.mod.Blocks.RAILWAY_SIGN_BOTTOM_MIDDLE.get()) && (facing == direction || allowOpposite && facing == direction.getOpposite())) {
					return checkPos;
				}
			} else {
				return null;
	...
	public static class BlockEntity extends BlockEntityExtension {
		...
		private static BlockEntityType<? extends BlockEntityExtension> getType(int length, boolean isOdd) {
			switch (length) {
				case 2:
					return isOdd ? BlockEntityTypes.RAILWAY_SIGN_BOTTOM_2_ODD.get() : BlockEntityTypes.RAILWAY_SIGN_BOTTOM_2_EVEN.get();
				case 3:
					return isOdd ? BlockEntityTypes.RAILWAY_SIGN_BOTTOM_3_ODD.get() : BlockEntityTypes.RAILWAY_SIGN_BOTTOM_3_EVEN.get();
				case 4:
					return isOdd ? BlockEntityTypes.RAILWAY_SIGN_BOTTOM_4_ODD.get() : BlockEntityTypes.RAILWAY_SIGN_BOTTOM_4_EVEN.get();
				case 5:
					return isOdd ? BlockEntityTypes.RAILWAY_SIGN_BOTTOM_5_ODD.get() : BlockEntityTypes.RAILWAY_SIGN_BOTTOM_5_EVEN.get();
				case 6:
					return isOdd ? BlockEntityTypes.RAILWAY_SIGN_BOTTOM_6_ODD.get() : BlockEntityTypes.RAILWAY_SIGN_BOTTOM_6_EVEN.get();
				case 7:
					return isOdd ? BlockEntityTypes.RAILWAY_SIGN_BOTTOM_7_ODD.get() : BlockEntityTypes.RAILWAY_SIGN_BOTTOM_7_EVEN.get();
				default:
					return BlockEntityTypes.RAILWAY_SIGN_BOTTOM_2_EVEN.get();
			}
		}
	}
}
