...
public class RenderRailwaySignBottom<T extends BlockRailwaySignBottom.BlockEntity> extends BlockEntityRenderer<T> implements IBlock, IGui, IDrawing {

	public RenderRailwaySignBottom(Argument dispatcher) {
		super(dispatcher);
	}
	...
	public void render(T entity, float tickDelta, GraphicsHolder graphicsHolder, int light, int overlay) {
		...
		if (!(state.getBlock().data instanceof BlockRailwaySignBottom)) {
			return;
		}
		final BlockRailwaySignBottom block = (BlockRailwaySignBottom) state.getBlock().data;
		...
		if (renderBackground) {
			...
			RenderTrains.scheduleRender(new Identifier(Init.MOD_ID, "textures/block/white.png"), false, RenderTrains.QueuedRenderLayer.LIGHT, (graphicsHolderNew, offset) -> {
				...
				IDrawing.drawTexture(graphicsHolderNew, 4, 0, SMALL_OFFSET, 0.5F * (signIds.length), 0.5F, SMALL_OFFSET, facing, newBackgroundColor, MAX_LIGHT_GLOWING);
	...
	public static void drawSign(GraphicsHolder graphicsHolder, @Nullable StoredMatrixTransformations storedMatrixTransformations, BlockPos pos, String signId, float x, float y, float size, float maxWidthLeft, float maxWidthRight, LongAVLTreeSet selectedIds, Direction facing, int backgroundColor, DrawTexture drawTexture) {
		...
		final float signSize = (sign.getSmall() ? BlockRailwaySignBottom.SMALL_SIGN_PERCENTAGE : 1) * size;
		...
		if (storedMatrixTransformations != null && isExit) {
			...
			for (int i = 0; i < selectedExitsSorted.size(); i++) {
				...
				RenderTrains.scheduleRender(DynamicTextureCache.instance.getExitSignLetter(selectedExit.substring(0, 1), selectedExit.substring(1), backgroundColor).identifier, true, RenderTrains.QueuedRenderLayer.LIGHT_TRANSLUCENT, (graphicsHolderNew, offset) -> {
					...
					IDrawing.drawTexture(graphicsHolderNew, signOffset, 4, signSize, signSize, facing, MAX_LIGHT_GLOWING);
					graphicsHolderNew.pop();
				});
			...
			if (hasCustomText) {
				final float fixedMargin = size * (1 - BlockRailwaySignBottom.SMALL_SIGN_PERCENTAGE) / 2;
